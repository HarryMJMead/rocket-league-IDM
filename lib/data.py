...
from lib.data_modification import just_player, rel_velocity_just_player, remove_on_ground, add_change, add_change_unmodified, corrupt
...
class EpisodeDataset_Unmodified(Dataset):
    def __init__(self, data_paths, width: int, include_change = False, corrupt=False):
        self.episodes = []
    ...
    def __getitem__(self, idx):
        ...
        obs = just_player(obs)
        if self.include_change:
            obs, act, act_num, add_data = add_change_unmodified(obs, act, act_num, add_data)
